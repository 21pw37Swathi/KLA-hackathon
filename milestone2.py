...
def calculate_die_coordinates(wafer_diameter, die_size, die_shift_vector, reference_die, shift_center):
    ...
    for i in range(0, -1*radius, -1*die_size[0]):
        for j in range(0, -1*radius, -1*die_size[1]):
            ...
            cow = (0, 0)
            if (is_point_inside_circle((llc_x, llc_y), cow, wafer_diameter)):
                die_index = (new_i - reference_die[0]+shift_center[0], new_j - reference_die[1]+shift_center[1])
                die_coordinates.append((die_index, (llc_x, llc_y)))
...
die_coordinates = calculate_die_coordinates(wafer_diameter, die_size, die_shift_vector, reference_die, shift_center)
